function [ x, info, perf ] = ...
    damp_newton( fg_handle, G_handle, x0, Std, Rule, varargin )
% DAMP_NEWTON.M finds the answer to problem P: x* = argmin f(x) using
...
% 
% Call
% [x,info,perf] = damp_newton(fg_handle, G_handle, x0)
% [x,info,perf] = damp_newton(fg_handle, G_handle, x0, Std)
% [x,info,perf] = damp_newton(fg_handle, G_handle, x0, Std, Rule)
% [x,info,perf] = damp_newton(fg_handle, G_handle, x0, Std, Rule, ...
...
while ( delta > Std(1) || g_norm > Std(2) ) && i < Std(3)
    ...
    e = eig(G);
    min_e = min(e);
    condG = max(abs(e))/min(abs(e));
    
    % to see if G is invertible.
    if condG > 1e8
        fprintf(['Error: Hesse matrix is too ill-conditioned. '...
            '(cond(G)=%d)\n'...
    ...
    d = - G \ g;
    % to see if G is positive definite.
    if min_e < 0
        fprintf(['Warning: Hesse matrix is not positive definite. '...
            '(min_eig=%d)\n'...
            'Newton method continue with at iteration %d.\n'],...
            min_e, i)
        % Fixing operation:
        % d = sign(g' * G * g) .* d;
    end
    ...
    info(3) = info(3) + rst(3);
    % if no step length can be found of decreasing
    if rst(1) == -1
...
% output relevant information in the last iteration.
perf.f_rec(i+1) = f;
