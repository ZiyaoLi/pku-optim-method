function [ x, info, perf ] = ...
    dogleg( frj_handle, Method, x0, Std, Delta, varargin )
% DOGLEG.M finds the answer to the least square problem 
...
%               Jacobian matrix.
% Method:       indicator of the Dogleg method used, scalar.
%               1 - Simple Dogleg method
%               2 - Two-fold Dogleg method
...
% 
% Call
% [x,info,perf] = dogleg(frj_handle, Method, x0)
% [x,info,perf] = dogleg(frj_handle, Method, x0, Std)
% [x,info,perf] = dogleg(frj_handle, Method, x0, Std, Delta)
% [x,info,perf] = dogleg(frj_handle, Method, x0, Std, Delta, p1, p2, ...)
...
while ( delta > eps1 || g_norm > eps2 ) && iter < maxiter % 
    ...
    if norm(dgn, 2) <= Delta
        ...
        norm_dsd = g_norm;
        tmp = J * dsd;
        alpha = norm_dsd * norm_dsd / (tmp' * tmp);
        adsd = alpha .* dsd;
        nadsd = norm(adsd, 2);
        if nadsd >= Delta
            d = (Delta / norm_dsd) .* dsd;
            flag = 1;
        else
            if Method == 2
                tmp = J * g;
                gm = (g_norm) ^ 4 / (tmp' * tmp) / (g' * (-dgn));
                eta = 0.8 * gm + 0.2;
                dgn = eta .* dgn;
            end
            deltad = dgn - adsd;
            qa = deltad' * deltad;
            qb = adsd' * deltad;
            qc = nadsd * nadsd - Delta * Delta;
            bac = sqrt(qb * qb - qa * qc);
            b1 = (-qb - bac) / qa;
            b2 = (-qb + bac) / qa;
            beta = max(b1, b2);
            d = (1 - beta) .* adsd + beta .* dgn;
            flag = 1;
        end
    end
    ...
    tmp = J * d;
    dq = -.5 * (tmp' * tmp) - tmp' * r;
    if dq == 0  % no delta in q can be observed, due to too small d 
        % and potentially too small Delta.
        fprintf(['No reliable step can be derived at iteration %d. ' ...
                 'Dogleg Method stop. \n'], iter)
        info(1) = -1;
        break
    end
...
info(2) = iter;
info(3) = iter + count; % reducing count++ in the loop by directly adding
