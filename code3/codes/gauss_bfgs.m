function [ x, info, perf ] = ...
    gauss_bfgs( frj_handle, x0, Std, Rule,  varargin )
% GAUSS_BFGS.M finds the answer to the least square problem 
%     P: x* = argmin f(x) = \sum_{i=1}^{m} r_i(x)
% using gauss-newton + bfgs method. 
% 
% Input 
...
% 
% Call
% [x,info,perf] = gauss_bfgs(frj_handle, x0)
% [x,info,perf] = gauss_bfgs(frj_handle, x0, Std)
% [x,info,perf] = gauss_bfgs(frj_handle, x0, Std, Rule)
% [x,info,perf] = gauss_bfgs(frj_handle, x0, Std, Rule, p1, p2, ...)
...
while ( delta > eps1 || g_norm > eps2 ) && iter < maxiter %
    ...
    numer = yhat - B * s;
    numer_mat = numer * y';
    B = B + denom .* (numer_mat + numer_mat') ...
          - ((denom * denom * numer' * s) .* y) * y';
